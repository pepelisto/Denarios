...
def close_position(s, po, close_date_, sl_tp_ratio, sl_limit, sl_low_limit, factor_ajuste, close_method):
    ...
    Closed_position_sim.objects.create(
        ...
        sim_info='histograma creciente, shigh20 y low20 for sl ,  '
                 'ajuste 2 golden egg el sl factor, q 100 ',
    )
        ...
        if type_ == 'BUY':
            ...
            if alteraciones == 0:
                if aumento > factor_ajuste:
                    ajuste = True
                    stop_loss = sl_p + po.entry_price * (factor_ajuste / 2) * 3
                    take_profit = tp_p + po.entry_price * (factor_ajuste / 2)
            else:
                if aumento > factor_ajuste + (alteraciones + 1) * (factor_ajuste / 2):
                    ajuste = True
                    stop_loss = sl_p + po.entry_price * (factor_ajuste / 2)
                    take_profit = tp_p + po.entry_price * (factor_ajuste / 2)
...
def simulator():
    ...
    for s in symbols:
        ...
        i += 1
        symbol = s.symbol
        csv_file_path = f"{path}{symbol}_simulation_with_indicators_2.csv"
