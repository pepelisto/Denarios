...
def close_position(s, po, close_date_, sl_tp_ratio, sl_limit, sl_low_limit, factor_ajuste, close_method):
    ...
    Closed_position_sim.objects.create(
        ...
        ratr=factor_ajuste,
        simulation=12,
        sim_info='con 5m timeframe anastasia, la diferencia es q el ajuste es con el close, '
                 'osea revisado solo una vez al inicio del periodo de 5m, ademas tiene el rsi en 90 y 20',
    )
...
def anastasia(s, symbol, df, df5, idx, sl_tp_ratio, sl_limit, sl_low_limit, factor_ajuste):
    ...
    for i in range(matching_row, matching_row - 144, -1):
        row_data = df5.iloc[i]
        type_ = po.type
        high = row_data['High']
        low = row_data['Low']
...
def simulator():
    ...
    for s in symbols:
        ...
        for v1 in [0]:#quedo fijado en 80 y 20, pq la variacion no mostro impacto signifiactivo
            stoch_buy = round(0.1 - v1, 2)
            stoch_sell = round(0.9 + v1, 2)
