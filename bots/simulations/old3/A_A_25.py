...
def create_position(symbol_, type_, entry_price_, quantity_, open_date_, stoch_, rsi_, sl_price, sl_tp_ratio):
    ...
    tp__ = entry_price_ + entry_price_ * sl_factor * (-1) * sl_tp_ratio
    sl__ = entry_price_ + entry_price_ * sl_factor
...
""" 
this simulation: rsi 56 period, stochastic rsi standard y macd standard, cuando el st rsi sobre 80, macd turns red 
( then try whe deacreasing two in a row ) and RSI is in down trend, the open position for sell, 

close it when get 1 usd profit, 2 usd loss, 

this to try later on: close ...or the rsi stoch reached bellow 20 or some thing, also adjust every 50 cents after de 1 usd mark
"""
