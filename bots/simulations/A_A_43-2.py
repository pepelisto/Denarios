...
def close_position(s, po, close_date_, sl_tp_ratio, sl_limit, sl_low_limit, factor_ajuste, close_method):
    ...
    Closed_position_sim.objects.create(
        ...
        ratr=factor_ajuste,
        simulation=443560002,
        sim_info='histograma creciente, con factor de ajuste inicial en stoplossfacto dividido 2, segundo stop loss facto and then 0.75',
    )
...
def anastasia(s, symbol, df, df5, idx, sl_tp_ratio, sl_limit, sl_low_limit, factor_ajuste):
    ...
    for i in range(matching_row, matching_row - 48, -1):
        ...
        if type_ == 'BUY':
            ...
            if alteraciones == 0:
                if aumento > (- factor_ajuste_inicial) / 2:
                    ajuste = True
                    stop_loss = sl_p + po.entry_price * (-factor_ajuste_inicial)
                    take_profit = tp_p + po.entry_price * (-factor_ajuste_inicial)
            elif alteraciones == 1:
                if aumento > (-factor_ajuste_inicial):
                    ajuste = True
                    stop_loss = sl_p + po.entry_price * (-factor_ajuste_inicial) * 1.5
                    take_profit = tp_p + po.entry_price * (-factor_ajuste_inicial) * 1.5
            else:
                if aumento > (-factor_ajuste_inicial) + (alteraciones - 1) * factor_ajuste:
                    ajuste = True
