...
def anastasia(s, symbol, df, idx, sl_tp_ratio, sl_limit, sl_low_limit):
    ...
    if type_ == 'BUY':
        ...
        if tp_period and not sl_period:
            ...
            aumento = (high / po.entry_price) - 1
            if aumento > factor:
               po.sl_price = po.entry_price + po.entry_price * 0.00036 * 2
               po.save()
        elif sl_p == po.entry_price + po.entry_price * 0.00036 * 2:
            factor = (((po.tp_price / po.entry_price) - 1) / sl_tp_ratio)
            aumento = (high / po.entry_price) - 1
            if aumento > factor * 2:
                po.sl_price = po.entry_price + po.entry_price * 0.00036 * 2 + po.entry_price * factor
                po.save()
        ...
        if tp_period and not sl_period:
            ...
            aumento = (-1)*((low / po.entry_price) - 1)
            if aumento > factor:
                po.sl_price = po.entry_price - po.entry_price * 0.00036 * 2
                po.save()
        elif sl_p == po.entry_price - po.entry_price * 0.00036 * 2:
            factor = (((po.tp_price / po.entry_price) - 1) / sl_tp_ratio)
            aumento = (low / po.entry_price) - 1
            if aumento < factor * 2:
                po.sl_price = po.entry_price + po.entry_price * 0.00036 * 2 + po.entry_price * factor
                po.save()
