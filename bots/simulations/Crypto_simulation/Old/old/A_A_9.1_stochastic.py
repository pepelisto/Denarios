...
def create_position(symbol_, type_, entry_price_, quantity_, open_date_, srsi_, rsi_, sl_price, sl_tp_ratio, atr, ratr):
...
def agripina(s, symbol, df, srsi_buy, srsi_sell, rsi_buy, rsi_sell, idx, sl_tp_ratio, sl_limit, sl_low_limit):
    ...
    if op.macd and op.rsi and op.stock_rsi:
        ...
        if abs(sl_factor) * sl_tp_ratio > sl_low_limit:
            ...
            create_position(symbol_, type_, entry_price_, quantity_, open_date_, srsi_, rsi_, sl_price, sl_tp_ratio, atr, ratr)
            update_opportunities(op, type='OPEN')
        else:
            update_opportunities(op, type='NONE', stock_rsi=False, macd=False, rsi=False)

def simulator():
    path = "samples/USDT/2023_ss/"
    symbols = Symbol.objects.filter(find_in_api=True)[21:]
    for s in symbols:
