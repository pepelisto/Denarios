...
def agripina(s, symbol, df,  stoch_buy, stoch_sell, rsi_buy, rsi_sell, idx, sl_tp_ratio, sl_limit, sl_low_limit):
    ...
    if op.type == 'SELL':
        if srsik <= 0.20 or srsid <= 0.20:
            DB_habndle.update_opportunities(op, type='NONE', stock_rsi=False, macd=False, rsi=False, var_1=0)
        else:
            ...
            if not op.var_1 == 1 and ema_100 > close:
                DB_habndle.update_opportunities(op, var_1=1)
            elif op.var_1 == 1 and ema_100 < close:
                DB_habndle.update_opportunities(op, var_1=0)
...
def simulator():
    ...
    symbols = Symbol.objects.filter(find_in_api=True, symbol='BTCUSDT')
    i = 1
    for s in symbols:
