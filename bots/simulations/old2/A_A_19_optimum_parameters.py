    ...
    if op.macd and op.rsi and op.stock_rsi:
        ...
        if op.type == 'BUY':
            ...
            if abs(sl_factor) > sl_limit_buy:
                ...
                sl_price = entry_price_ * (1 - sl_low_limit_buy)
            ...
            if sl_factor > sl_limit_sell:
                ...
                sl_price = entry_price_ * (1 + sl_low_limit_sell)
...
def get_parameters(s, df):
    start_date = datetime(2023, 1, 1)
    end_date = datetime(2023, 6, 30)
    best_buy = Closed_position_sim.objects.values(
            'symbol__symbol', 'type', 'simulation', 'tp_sl_ratio', 'sl_low_limit', 'sl_limit', 'rsi_open', 'stoch_open'
            ).filter(
    ...
    best_sell = Closed_position_sim.objects.values(
            'symbol__symbol', 'type', 'simulation', 'tp_sl_ratio', 'sl_low_limit', 'sl_limit', 'rsi_open', 'stoch_open'
            ).filter(
                  symbol__symbol=s.symbol,
                  close_date__range=(start_date, end_date),
    ...
    pnl_buy = best_buy['pnl_total']
    pnl_sell = best_sell['pnl_total']
    stoch_buy = best_buy['stoch_open']
    stoch_sell = best_sell['stoch_open']
    rsi_buy = best_buy['rsi_open']
    rsi_sell = best_sell['rsi_open']
    sl_tp_ratio_buy = best_buy['tp_sl_ratio']
    sl_tp_ratio_sell = best_sell['tp_sl_ratio']
    sl_low_limit_buy = best_buy['sl_low_limit']
    sl_low_limit_sell = best_sell['sl_low_limit']
    sl_limit_buy = best_buy['sl_limit']
    sl_limit_sell = best_sell['sl_limit']
...
def simulator():
    ...
    for s in symbols:
        ...
        stoch_buy, stoch_sell, rsi_buy, rsi_sell, sl_tp_ratio_buy, sl_tp_ratio_sell, sl_limit_buy, sl_limit_sell, \
            sl_low_limit_buy, sl_low_limit_sell, pnl_buy, pnl_sell = get_parameters(s, df)
        if pnl_buy < 0 and pnl_sell < 0:
            continue
        for idx in range(num_rows - 150, -1, -1):
            if pd.Timestamp(df.loc[idx, 'timestamp']) < pd.Timestamp(2023, 7, 1):
                continue
