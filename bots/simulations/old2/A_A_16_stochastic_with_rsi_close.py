...
def close_position(s, po, close_date_, sl_tp_ratio, sl_limit, sl_low_limit, close_method, close_price=None):
    if close_method == "TP":
        exit_price = po.tp_price
    elif close_method == "SL":
...
def anastasia(s, symbol, df, idx, sl_tp_ratio, sl_limit, sl_low_limit):
    ...
    if type_ == 'BUY':
        ...
        if tp_period and not sl_period:
            ...
            if close > po.entry_price:
                close_position(s, po, close_date_, sl_tp_ratio, sl_limit, sl_low_limit, close_method='TPrsi', close_price=close)
                return
            if close < po.entry_price:
                close_position(s, po, close_date_, sl_tp_ratio, sl_limit, sl_low_limit, close_method='SLrsi', close_price=close)
                return
...
def simulator():
    ...
    symbols = Symbol.objects.filter(find_in_api=True, symbol='XRPUSDT')
    for s in symbols:
        ...
        for v1 in [0]:
            ...
            for v2 in [0]:
                ...
                for v3 in [3, 4]:
                    sl_tp_ratio = v3
                    for v5 in [0.005]:#, 0.007, 0.001]:# este quedo fijoo para siempre
                        sl_low_limit = v5
                        for v4 in [0.045]:
                            sl_limit = v4
                            print(str(v3) + '  ' + str(v4))
