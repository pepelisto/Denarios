...
def close_position(s, po, symbol, exit_price, close_date_, sl_tp_ratio, close_method):
    if close_method == "TP":
...
def anastasia(s, symbol, df, idx, sl_tp_ratio):
    try:
    ...
    if type_ == 'BUY':
        if exit_price >= tp_p:
            close_position(s, po, symbol, exit_price, close_date_, sl_tp_ratio, close_method='TP')
        elif exit_price <= sl_p:
            close_position(s, po, symbol, exit_price, close_date_, sl_tp_ratio, close_method='SL')
        elif sl_p != po.entry_price + po.entry_price * 0.00027 * 2:
            factor = (abs((sl_p / po.entry_price) - 1)) * (sl_tp_ratio / 2)
            aumento = (exit_price / po.entry_price) - 1
            if aumento > factor:
               print('precio BUY ajustado')
               po.sl_price = po.entry_price + po.entry_price * 0.00027 * 2
               po.save()
    else:
        if exit_price <= tp_p:
            close_position(s, po, symbol, exit_price, close_date_, sl_tp_ratio, close_method='TP')
...
def simulator():
    ...
    # symbols = [Symbol.objects.get(symbol='ETHBUSD')]
    ...
    for s in symbols:
        ...
        for v1 in [-0.05, 0, 0.05, 0.1]:
            ...
            for v2 in [0, 2, 4]:
                rsi_buy = 50 + v2
                rsi_sell = 50 - v2
                for v3 in [1, 1.5, 2, 2.5, 3]:
                    sl_tp_ratio = v3      # simular con varios ratios distintos
                    for idx in range(num_rows - 150, -1, -1):
                        anastasia(s, symbol, df, idx, sl_tp_ratio)
