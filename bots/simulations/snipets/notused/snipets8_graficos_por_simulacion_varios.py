...
for s in list:
    ...
    simulations = Closed_position_sim.objects.filter(simulation=simulation, rsi_open=rsi, tp_sl_ratio=tp_sl_ratio).filter(
        close_date__range=(star_date, end_date)).values_list('simulation', flat=True).distinct()
