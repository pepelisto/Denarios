...
result = Closed_position_sim.objects.values(
    ...
    'simulation',
         ...
         simulation__startswith=450000000,
).annotate(
    positions=Count('id'),
...
simulations = Closed_position_sim.objects.filter(simulation=450000000).filter(close_date__range=(star_date, end_date)).values_list('simulation', flat=True).distinct()
