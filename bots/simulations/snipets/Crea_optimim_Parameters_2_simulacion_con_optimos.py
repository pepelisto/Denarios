...
for r in [0,1,2]:
    star_date = datetime(2020 + r, 1, 1)
    end_date = datetime(2020 + r, 12, 30)
    star_date_s = datetime(2021 + r, 1, 1)
    end_date_s = datetime(2021 + r, 12, 30)
    pnl = 0
    for i in range(0, 30):
        ...
        for entry in result:
            print(entry)
            s = Closed_position_sim.objects.values(
                'symbol__symbol',  # 'type',#, 'tp_sl_ratio', 'sl_limit' 'rsi_open', 'stoch_open',
                'simulation',
                'tp_sl_ratio', 'sl_limit', 'sl_low_limit', 'ratr',
                'simulation',
            ).filter(close_date__range=(star_date_s, end_date_s)).filter(symbol_id=i).filter(simulation=entry['simulation']).annotate(
                positions=Count('id'),
                pnl_total=Sum('profit'),
                positive_pnl_count=Count(Case(When(profit__gt=0, then=1))),
                negative_pnl_count=Count(Case(When(profit__lt=0, then=1))),
