...
def agripina(s, symbol, df, stoch_buy, stoch_sell, rsi_buy, rsi_sell, idx, sl_tp_ratio, sl_limit, sl_low_limit):
    ...
    if op.macd and op.rsi and op.stock_rsi:
        ...
        symbol_ = s
        if op.type == 'BUY':
            sl_price = max(calculate_swing_low(df, idx), df.loc[idx, 'ema_50'])
            ...
            sl_price = min(calculate_swing_high(df, idx), df.loc[idx, 'ema_50'])
                ...
                for v3 in [3]:
                    ...
                    for v5 in [0.0015]:
                        ...
                        for v4 in [0.04]:
                            sl_limit = v4
                            print(str(v3) + '  ' + str(v4))
                            for idx in range(num_rows - 50000, -1, -1):
