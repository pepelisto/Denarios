...
def close_position(s, po, close_date_, sl_tp_ratio, sl_limit, sl_low_limit, close_method):
    ...
    Open_position.objects.get(symbol_id=s.symbol.pk, timeframe=15).delete()
...
def traeder():
    ...
    if not open_positions.exists():
        return
    symbols = Optimum_parameter.objects.filter(symbol__id__in=open_positions, timeframe=15).order_by('-pnl')
    for s in symbols:
        ...
        Agripina.anastasia(s, df, sl_tp_ratio, sl_limit, sl_low_limit)
