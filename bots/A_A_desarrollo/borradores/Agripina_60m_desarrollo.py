...
def create_position(symbol_, type_, entry_price_, quantity_, open_date_, stoch_, rsi_, sl_price, take_profit, leverage):
...
def open_position(symbol, side, stop_loss_factor, take_profit_factor, usdt_size):
    ...
    return entry_price, stop_loss, take_profit, leverage
...
def agripina(s, df, stoch_buy, stoch_sell, rsi_buy, rsi_sell, sl_tp_ratio, sl_limit, sl_low_limit):
    ...
    if op.type == 'OPEN':
        return
    # macdhistogram = df['MACD histogram'].iloc[0]
    # srsik = df['St k'].iloc[0]
    # srsid = df['St d'].iloc[0]
    # rsi = df['RSI'].iloc[0]
    ...
    entry_price, stop_loss, take_profit, leverage = open_position(sym, type_, loss_factor, profit_factor, usdt_size)
    # create_position(symbol_, type_, entry_price, quantity_, open_date_, stoch_, rsi_, stop_loss, take_profit, leverage)
    # update_opportunities(op, type='OPEN')

def traeder():
    symbols = Optimum_parameter.objects.filter(timeframe=60).order_by('-pnl')[1:2]
    for s in symbols:
